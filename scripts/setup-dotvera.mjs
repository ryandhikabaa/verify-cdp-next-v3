...
const schemaPath = path.join(projectRoot, 'database', 'dotvera.sql');
...
const targetDatabase = process.env.DOTVERA_DB_NAME || 'dotvera';
...
async function main() {
  ...
  await applySchema();
  console.log('Dotvera setup completed.');
}

main().catch((error) => {
  console.error('Failed to set up Dotvera database:', error);
  process.exitCode = 1;
});